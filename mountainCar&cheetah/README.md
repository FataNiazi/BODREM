...
Run CLI commands from the parent workspace directory (`/Users/kevaanbuch/Desktop/Uni/CSC415`) so `python -m mountaincar...` resolves correctly.
...
Run one quick MountainCar smoke experiment (all methods, one seed):
...
Run one quick HalfCheetah smoke experiment (one method, one seed):
...
## Rerunning Experiments (Minimal)

Rerun the same method/seed from scratch:
...
python -m mountaincar.cli run \
  ...
  --task mountaincar \
  --init-mode scratch \
  --output-root ./runs/reruns
```

Rerun using a pretrained checkpoint:
...
Resume a previous batch and rerun only failed attempts:
...
python -m mountaincar.cli batch-run \
  ...
  --profile smoke \
  --task mountaincar \
  --resume-from ./runs/batch_run_mountaincar_mountaincar_v0_native_dqn_target_solve_rate_YYYYMMDD_HHMMSS/manifest.json \
  --rerun-failed
```

Note: `--resume-from` accepts either a batch directory path or a file inside it (for example `manifest.json`).

## Plotting Results

Generate paper-style plots from a method run, suite directory, batch directory, or any directory tree that contains `history.csv` artifacts:

```bash
python -m mountaincar.plot_results \
  --input ./runs/all_methods_halfcheetah_halfcheetah_v4_sb3_sac_mean_episodic_return_20260406_025639 \
  --output ./runs/plots/halfcheetah_smoke
```

Filter the plot set to selected methods or a single task:

```bash
python -m mountaincar.plot_results \
  --input ./runs \
  --output ./runs/plots/mountaincar_only \
  --methods BO,DR,BO+DR \
  --task mountaincar
```

Set a custom `g_last` reference line for DORAEMON-style methods:

```bash
python -m mountaincar.plot_results \
  --input ./runs/halfcheetah_seq_1800 \
  --output ./runs/plots/halfcheetah_alpha_09 \
  --task halfcheetah \
  --alpha-target 0.9
```

Generated outputs include per-method overview figures, centre-trajectory figures, PNG/PDF copies of every plot, and `plot_manifest.json` describing the emitted files.
